...
function buildEmailHtml(
  clientName: string,
  serviceLabel: string,
  request: { title: string; admin_reply: string | null },
  downloadUrl: string,
  fileName: string
): string {
  return `
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const subject = `Your ${serviceLabel} project "${request.title}" has been delivered! 🎉`;

    await sendGmail(clientEmail, subject, emailHtml);
